...
def afficher_progressivement(texte, delai, tag):
    for i, caractere in enumerate(texte):
        ...
        time.sleep(delai)
        if i % 10 == 0:  # Rafraîchir l'affichage toutes les 10 caractères pour une meilleure expérience utilisateur
            dialogue_text.update()
...
def soumettre_question():
    ...
    if question.strip() != "":
        ...
        dialogue_text.insert(tk.END, "Assistant : ", (COULEUR_ASSISTANT,))
        dialogue_text.update()
        afficher_progressivement(reponse, 0.02, COULEUR_ASSISTANT)
        dialogue_text.insert(tk.END, "\n\n", (COULEUR_ASSISTANT,))
        dialogue_text.see(tk.END)  # Défiler automatiquement vers la fin du texte
...
fenetre = tk.Tk()
fenetre.title("Assistant virtuel")
...
soumettre_bouton = ttk.Button(saisie_frame, text="Soumettre", command=soumettre_question, style="TButton")
